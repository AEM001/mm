...
try:
    ...
    if available_font is None:
        ...
        # 使用找到的字体
        plt.rcParams['font.sans-serif'] = [available_font.get_name()]
        font = available_font
...
def visualize_path(path, speed_plan, title):
    ...
    try:
        ...
        # 使用找到的字体设置标签
        cbar.set_label('超速比例 (%)', fontproperties=font, fontsize=12)
        ...
        plt.xlabel('列（从左到右）', fontproperties=font, fontsize=12)
        plt.ylabel('行（从下到上）', fontproperties=font, fontsize=12)
        plt.grid(True)
        plt.legend(prop={'family': font.get_name() if hasattr(font, 'get_name') else 'SimHei', 'size': 12})
        
        # 确保输出目录存在
        output_dir = os.path.dirname(os.path.abspath(__file__))
        # 创建安全的文件名
        safe_filename = "path_visualization.png"
...
def main():
    try:
        # 指定的路径（原图编号）
        path_numbers = [1, 2, 12, 13, 14, 24, 25, 35, 45, 55, 56, 57, 58, 59, 69, 79, 89, 90, 100]
        ...
        # 可视化路径
        success = visualize_path(path_indices, speed_plan, f"问题一路径（总时间 T1: {T1} 小时）")
        
        if success:
            print(f"路径可视化完成！")
            print(f"路径节点序列（原图编号）: {path_numbers}")
            print(f"路径长度: {len(path_numbers)}个节点，{len(path_numbers)-1}段路")
            print(f"总时间 T1: {T1} 小时")
        else:
            print("路径可视化失败！")
    except Exception as e:
        print(f"主函数错误: {e}")
