...
class IntervalCensoredSurvivalModel:
    ...
    def perform_bmi_clustering(self, n_clusters=None):
        ...
        print(f"特征统计信息:")
        print(f"  BMI范围: [{self.data['BMI'].min():.2f}, {self.data['BMI'].max():.2f}]")
        print(f"  达标时刻范围: [{self.data['事件时间估计'].min():.2f}, {self.data['事件时间估计'].max():.2f}]")
        ...
        print(f"\n二维聚类分组结果:")
        print(self.bmi_groups[['组别', 'BMI区间', '达标时刻区间', 'BMI均值', '达标时刻均值(周)', '样本数']])
    ...
    def generate_report(self):
        ...
        # 计算达标时间的原始孕周范围
        time_min_weeks = self.standardized_to_original_time(self.data['事件时间估计'].min())
        time_max_weeks = self.standardized_to_original_time(self.data['事件时间估计'].max())
...
## 1. 数据概况
- 总样本数: {len(self.data)}
- BMI范围: {self.data['BMI'].min():.1f} - {self.data['BMI'].max():.1f} (标准化值)
- 达标时间范围: {time_min_weeks:.1f} - {time_max_weeks:.1f}周
...
"""
        ...
        for _, group in self.bmi_groups.iterrows():
            report += f"- 第{group['组别']}组: BMI {group['BMI区间']}, 达标时刻 {group['达标时刻区间']}, 样本数 {group['样本数']}\n"
